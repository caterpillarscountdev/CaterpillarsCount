...
header('Access-Control-Allow-Origin: *');
	...
	$dbconn = (new Keychain)->getDatabaseConnection();
      $query = mysqli_query($dbconn, "SELECT COUNT(Survey.ID) AS Count FROM Survey JOIN Plant ON Survey.PlantFK=Plant.ID WHERE `SiteFK`='" . $site->getID() . "' AND Survey.LocalDate>'" . date("Y") . "-06-13'");
