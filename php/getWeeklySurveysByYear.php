<?php
	...
	if(is_object($user) && get_class($user) == "User"){
		...
		$query = mysqli_query($dbconn, "SELECT Plant.SiteFK, WEEK(Survey.LocalDate, 1) AS Week, COUNT(*) AS SurveyCount FROM Survey JOIN Plant ON Survey.PlantFK=Plant.ID WHERE YEAR(Survey.LocalDate)='$year' AND Plant.SiteFK IN (" . implode(", ", $siteIDsThisIteration) . ") GROUP BY Plant.SiteFK, WEEK(Survey.LocalDate, 1)");
