<?php
  ...
  function haveDeletedOldFiles(){
    global $files;
    for($i = 0; $i < count($files); $i++){
      if(strpos($files[$i], date("Y-m-d") . "_") !== false){
        return true;
      }
    }
    return false;
  ...
  if(getSave("allBackedUp", 100 * 365 * 24 * 60 * 60) != date("Y-m-d")){
    save("allBackedUp", date("Y-m-d"));
    $dbconn = (new Keychain)->getDatabaseConnection();
