	...
	if (is_array($siteIDs)) {
 	...
 	if($breakdown == "site" || $breakdown == "none"){
 		...
 		for($i = 0; $i < count($siteIDs); $i++){
			...
			if($HIGH_TRAFFIC_MODE){
 			...
 			if($comparisonMetric == "occurrence"){
				...
				$query = mysqli_query($dbconn, "SELECT ArthropodSighting.UpdatedGroup, COUNT(DISTINCT ArthropodSighting.SurveyFK) AS SurveysWithArthropodCount FROM `ArthropodSighting` JOIN Survey ON ArthropodSighting.SurveyFK = Survey.ID JOIN Plant ON Survey.PlantFK = Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK = '$siteID' GROUP BY ArthropodSighting.UpdatedGroup");
				while($row = mysqli_fetch_assoc($query)){
					$arthropodSurveys[$row["UpdatedGroup"]] = floatval($row["SurveysWithArthropodCount"]);
				}
				//surveys at site
				$query = mysqli_query($dbconn, "SELECT COUNT(*) AS TotalSurveyCount FROM `Survey` JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK='$siteID'");
				...
				$query = mysqli_query($dbconn, "SELECT ArthropodSighting.UpdatedGroup, SUM(ArthropodSighting.Quantity) AS ArthropodCount FROM ArthropodSighting JOIN Survey ON ArthropodSighting.SurveyFK=Survey.ID JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK='$siteID' GROUP BY ArthropodSighting.UpdatedGroup");
				while($row = mysqli_fetch_assoc($query)){
					$arthropodCounts[$row["UpdatedGroup"]] = floatval($row["ArthropodCount"]);
				}
				//total survey count at site
				$query = mysqli_query($dbconn, "SELECT COUNT(*) AS SurveyCount FROM Survey JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK='$siteID'");
				...
				$query = mysqli_query($dbconn, "SELECT ArthropodSighting.UpdatedGroup, ArthropodSighting.Length, SUM(ArthropodSighting.Quantity) AS TotalQuantity FROM `ArthropodSighting` JOIN Survey ON ArthropodSighting.SurveyFK = Survey.ID JOIN Plant ON Survey.PlantFK = Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK = '$siteID' GROUP BY ArthropodSighting.UpdatedGroup, ArthropodSighting.Length");
				while($row = mysqli_fetch_assoc($query)){
				...
				//total survey count at site
				$query = mysqli_query($dbconn, "SELECT SUM(ArthropodSighting.Quantity) AS AllArthropodsCount FROM ArthropodSighting JOIN Survey ON ArthropodSighting.SurveyFK=Survey.ID JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK='$siteID'");
		...
		if($comparisonMetric == "occurrence"){
 			...
 			$query = mysqli_query($dbconn, "SELECT DISTINCT $breakdownUpper(LocalDate) AS $breakdownTitle FROM Survey JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND SiteFK='$siteID'");
 			while($row = mysqli_fetch_assoc($query)){
				$arthropodSurveyCounts[strval($row[$breakdownTitle])] = array();
				$arthropodOccurrencesSet[strval($row[$breakdownTitle])] = array();
 			}
 			$query = mysqli_query($dbconn, "SELECT $breakdownUpper(Survey.LocalDate) AS $breakdownTitle, ArthropodSighting.UpdatedGroup, COUNT(DISTINCT ArthropodSighting.SurveyFK) AS ArthropodSurveyCounts FROM `ArthropodSighting` JOIN Survey ON ArthropodSighting.SurveyFK = Survey.ID JOIN Plant ON Survey.PlantFK = Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK = '$siteID' GROUP BY CONCAT($breakdownUpper(Survey.LocalDate), '-', ArthropodSighting.UpdatedGroup)");
 			while($row = mysqli_fetch_assoc($query)){
 				$arthropodSurveyCounts[strval($row[$breakdownTitle])][$row["UpdatedGroup"]] = $row["ArthropodSurveyCounts"];
 			}
 
 			$surveyCounts = array();
 			$query = mysqli_query($dbconn, "SELECT $breakdownUpper(Survey.LocalDate) AS $breakdownTitle, COUNT(Survey.ID) AS SurveyCount FROM Survey JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK='$siteID' GROUP BY $breakdownUpper(Survey.LocalDate)");
 			while($row = mysqli_fetch_assoc($query)){
			...
			if($HIGH_TRAFFIC_MODE){
 			...
 			$query = mysqli_query($dbconn, "SELECT $breakdownUpper(Survey.LocalDate) AS $breakdownTitle, ArthropodSighting.UpdatedGroup, SUM(ArthropodSighting.Quantity) AS ArthropodCount FROM ArthropodSighting JOIN Survey ON ArthropodSighting.SurveyFK=Survey.ID JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK='$siteID' GROUP BY CONCAT($breakdownUpper(Survey.LocalDate), '-', ArthropodSighting.UpdatedGroup)");
 			while($row = mysqli_fetch_assoc($query)){
			...
			$meanBiomassesSet = array();
 			...
 			$query = mysqli_query($dbconn, "SELECT $breakdownUpper(Survey.LocalDate) AS $breakdownTitle, ArthropodSighting.UpdatedGroup, ArthropodSighting.Length, SUM(ArthropodSighting.Quantity) AS TotalQuantity FROM `ArthropodSighting` JOIN Survey ON ArthropodSighting.SurveyFK = Survey.ID JOIN Plant ON Survey.PlantFK = Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK = '$siteID' GROUP BY $breakdownUpper(Survey.LocalDate), ArthropodSighting.UpdatedGroup, ArthropodSighting.Length");
 			while($row = mysqli_fetch_assoc($query)){
			...
			$arthropodRelativeProportionsSet = array();
 			$arthropodCounts = array();
 			$query = mysqli_query($dbconn, "SELECT DISTINCT $breakdownUpper(LocalDate) AS $breakdownTitle FROM Survey JOIN Plant ON Survey.PlantFK=Plant.ID Survey.ReviewedAndpproved < 3 AND WHERE SiteFK='$siteID'");
 			while($row = mysqli_fetch_assoc($query)){
				$arthropodCounts[strval($row[$breakdownTitle])] = array();
 			...
 			$query = mysqli_query($dbconn, "SELECT $breakdownUpper(Survey.LocalDate) AS $breakdownTitle, SUM(ArthropodSighting.Quantity) AS AllArthropodsCount FROM ArthropodSighting JOIN Survey ON ArthropodSighting.SurveyFK=Survey.ID JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK='$siteID' GROUP BY $breakdownUpper(Survey.LocalDate)");
 			while($row = mysqli_fetch_assoc($query)){
		...
		$totalDensity = array();
		$circleNums = array();
		$query = mysqli_query($dbconn, "SELECT Plant.$plantField, SUM(ArthropodSighting.Quantity) AS ArthropodCount FROM ArthropodSighting JOIN Survey ON ArthropodSighting.SurveyFK=Survey.ID JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK='$siteID' AND $extraWhere GROUP BY Plant.$plantField");
		while($row = mysqli_fetch_assoc($query)){
		...
		$query = mysqli_query($dbconn, "SELECT Plant.$plantField, COUNT(*) AS SurveyCount, COUNT(DISTINCT Plant.ID) AS Branches FROM Survey JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK='$siteID' AND $extraWhere GROUP BY Plant.$plantField");
		while($row = mysqli_fetch_assoc($query)){
		...
		if ($breakdown=='circle') { //sort by circle number
 		...
 		if($comparisonMetric == "occurrence"){
 			...
 			$query = mysqli_query($dbconn, "SELECT Plant.$plantField, ArthropodSighting.UpdatedGroup, COUNT(DISTINCT ArthropodSighting.SurveyFK) AS ArthropodSurveyCounts FROM `ArthropodSighting` JOIN Survey ON ArthropodSighting.SurveyFK = Survey.ID JOIN Plant ON Survey.PlantFK = Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK = '$siteID' AND $extraWhere GROUP BY CONCAT(Plant.$plantField, '-', ArthropodSighting.UpdatedGroup)");
 			while($row = mysqli_fetch_assoc($query)){
 				$arthropodSurveyCounts[$row[$plantField]][$row["UpdatedGroup"]] = $row["ArthropodSurveyCounts"];
 			}
 
 			$surveyCounts = array();
 			$query = mysqli_query($dbconn, "SELECT Plant.$plantField, COUNT(Survey.ID) AS SurveyCount FROM Survey JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK='$siteID' AND $extraWhere GROUP BY Plant.$plantField");
 			while($row = mysqli_fetch_assoc($query)){
			...
			$speciesKeys = array_keys($arthropodSurveyCounts);
 			...
 			$query = mysqli_query($dbconn, "SELECT Plant.$plantField, ArthropodSighting.UpdatedGroup, SUM(ArthropodSighting.Quantity) AS ArthropodCount FROM ArthropodSighting JOIN Survey ON ArthropodSighting.SurveyFK=Survey.ID JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK='$siteID' AND $extraWhere GROUP BY CONCAT(Plant.$plantField, '-', ArthropodSighting.UpdatedGroup)");
 			while($row = mysqli_fetch_assoc($query)){
			...
			$meanBiomassesSet = array();
 			...
 			$query = mysqli_query($dbconn, "SELECT Plant.$plantField, ArthropodSighting.UpdatedGroup, ArthropodSighting.Length, SUM(ArthropodSighting.Quantity) AS TotalQuantity FROM `ArthropodSighting` JOIN Survey ON  ArthropodSighting.SurveyFK = Survey.ID JOIN Plant ON Survey.PlantFK = Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK = '$siteID' AND $extraWhere GROUP BY Plant.$plantField, ArthropodSighting.UpdatedGroup, ArthropodSighting.Length");
 			while($row = mysqli_fetch_assoc($query)){
			...
			$arthropodRelativeProportionsSet = array();
 			...
 			$allArthropodCounts = array();
 			$query = mysqli_query($dbconn, "SELECT Plant.$plantField, SUM(ArthropodSighting.Quantity) AS AllArthropodsCount FROM ArthropodSighting JOIN Survey ON ArthropodSighting.SurveyFK=Survey.ID JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndpproved < 3 AND Plant.SiteFK='$siteID'  AND $extraWhere GROUP BY Plant.$plantField");
 			while($row = mysqli_fetch_assoc($query)){
