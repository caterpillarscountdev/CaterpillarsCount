<?php
	...
	if(is_object($user) && get_class($user) == "User"){
		...
		$query = mysqli_query($dbconn, "SELECT Plant.SiteFK, WEEK(Survey.LocalDate, 1) AS Week, COUNT(*) AS SurveyCount FROM Survey JOIN Plant ON Survey.PlantFK=Plant.ID WHERE YEAR(Survey.LocalDate)='$lastSurveyYear' AND Plant.SiteFK IN (" . implode(", ", $siteIDsThisIteration) . ") GROUP BY Plant.SiteFK, WEEK(Survey.LocalDate, 1)");
		...
		//Plants
		$query = mysqli_query($dbconn, "SELECT SiteFK, COUNT(*) AS PlantCount FROM Plant WHERE SiteFK IN (" . implode(", ", $siteIDsThisIteration) . ") AND Circle>'0' GROUP BY SiteFK");
		if(mysqli_num_rows($query) > 0){
			while($row = mysqli_fetch_assoc($query)){
				if(array_key_exists($row["SiteFK"], $data)){
