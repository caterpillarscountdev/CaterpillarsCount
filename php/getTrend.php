<?php
	...
	$readableArthropods = array(
  	...
  	for($i = 0; $i < count($lines); $i++){
		...
		$query = mysqli_query($dbconn, "SELECT YEAR(Survey.LocalDate) AS Year, COUNT(*) AS YearlySurveyCount FROM `Survey` JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Plant.SiteFK='$siteID' AND (MONTH(Survey.LocalDate)>='$startMonth' AND MONTH(Survey.LocalDate)<='$endMonth') GROUP BY YEAR(Survey.LocalDate) ORDER BY YEAR(Survey.LocalDate)");
		...
		$query = mysqli_query($dbconn, "SELECT YEAR(Survey.LocalDate) AS Year, COUNT(DISTINCT ArthropodSighting.SurveyFK) AS SurveysWithArthropodsCount FROM ArthropodSighting JOIN Survey ON ArthropodSighting.SurveyFK=Survey.ID JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Plant.SiteFK='$siteID' AND ArthropodSighting.UpdatedGroup LIKE '$arthropod' AND (MONTH(Survey.LocalDate)>='$startMonth' AND MONTH(Survey.LocalDate)<='$endMonth') GROUP BY YEAR(Survey.LocalDate) ORDER BY YEAR(Survey.LocalDate)");
		while($row = mysqli_fetch_assoc($query)){
		...
		$query = mysqli_query($dbconn, "SELECT YEAR(Survey.LocalDate) AS Year, SUM(ArthropodSighting.Quantity) AS YearlyArthropodSightings FROM `ArthropodSighting` JOIN Survey ON ArthropodSighting.SurveyFK=Survey.ID JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Plant.SiteFK='$siteID' AND ArthropodSighting.UpdatedGroup LIKE '$arthropod' AND (MONTH(Survey.LocalDate)>='$startMonth' AND MONTH(Survey.LocalDate)<='$endMonth') GROUP BY YEAR(Survey.LocalDate) ORDER BY YEAR(Survey.LocalDate)");
		while($row = mysqli_fetch_assoc($query)){
		...
		$query = mysqli_query($dbconn, "SELECT YEAR(Survey.LocalDate) AS Year, ArthropodSighting.UpdatedGroup, ArthropodSighting.Length, SUM(ArthropodSighting.Quantity) AS TotalQuantity FROM `ArthropodSighting` JOIN Survey ON ArthropodSighting.SurveyFK=Survey.ID JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Plant.SiteFK='$siteID' AND ArthropodSighting.UpdatedGroup LIKE '$arthropod' AND (MONTH(Survey.LocalDate)>='$startMonth' AND MONTH(Survey.LocalDate)<='$endMonth') GROUP BY YEAR(Survey.LocalDate), ArthropodSighting.UpdatedGroup, ArthropodSighting.Length");
		while($row = mysqli_fetch_assoc($query)){
