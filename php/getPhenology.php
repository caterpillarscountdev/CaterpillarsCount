<?php
	...
	$readableArthropods = array(
  	...
  	for($i = 0; $i < count($lines); $i++){
		...
		$query = mysqli_query($dbconn, "SELECT $dateStr, COUNT(*) AS SurveyCount FROM `Survey` JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndApproved < 3 AND Plant.SiteFK='$siteID' AND YEAR(Survey.LocalDate)='$year' GROUP BY SurveyDate ORDER BY SurveyDate");
		...
		$query = mysqli_query($dbconn, "SELECT $dateStr, COUNT(DISTINCT ArthropodSighting.SurveyFK) AS SurveysWithArthropodsCount FROM ArthropodSighting JOIN Survey ON ArthropodSighting.SurveyFK=Survey.ID JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndApproved < 3 AND Plant.SiteFK='$siteID' AND ArthropodSighting.UpdatedGroup LIKE '$arthropod' AND YEAR(Survey.LocalDate)='$year' GROUP BY SurveyDate ORDER BY SurveyDate");
		while($row = mysqli_fetch_assoc($query)){
		...
		$query = mysqli_query($dbconn, "SELECT $dateStr, SUM(ArthropodSighting.Quantity) AS ArthropodSightings FROM `ArthropodSighting` JOIN Survey ON ArthropodSighting.SurveyFK=Survey.ID JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndApproved < 3 AND Plant.SiteFK='$siteID' AND ArthropodSighting.UpdatedGroup LIKE '$arthropod' AND YEAR(Survey.LocalDate)='$year' GROUP BY SurveyDate ORDER BY SurveyDate");
		while($row = mysqli_fetch_assoc($query)){
		...
		$query = mysqli_query($dbconn, "SELECT $dateStr, ArthropodSighting.UpdatedGroup, ArthropodSighting.Length, SUM(ArthropodSighting.Quantity) AS TotalQuantity FROM `ArthropodSighting` JOIN Survey ON ArthropodSighting.SurveyFK=Survey.ID JOIN Plant ON Survey.PlantFK=Plant.ID WHERE Survey.ReviewedAndApproved < 3 AND Plant.SiteFK='$siteID' AND ArthropodSighting.UpdatedGroup LIKE '$arthropod' AND YEAR(Survey.LocalDate)='$year' GROUP BY SurveyDate, ArthropodSighting.UpdatedGroup, ArthropodSighting.Length");
		while($row = mysqli_fetch_assoc($query)){
