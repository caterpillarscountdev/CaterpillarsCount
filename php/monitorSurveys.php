<?php
	...
	if(mysqli_num_rows($query) > 0){
        ...
        foreach ($duplicatesObj as $row){
          $duplicates[] = "SURVEY ID: " . $row["ID"] . "| " . $row["Code"] . " " . $row["LocalDate"] . " " . $row["LocalTime"] . " " . $row["FullName"] . " possible duplicates: " . implode(",", $row["duplicates"]);
        }
          


        $body = "<html><body><p>There may have been a timeout or some other error that caused a user to have to submit the same survey twice. The following Survey IDs should be investigated:</p>" . implode("<p>", $duplicates) . "<p><p>";
